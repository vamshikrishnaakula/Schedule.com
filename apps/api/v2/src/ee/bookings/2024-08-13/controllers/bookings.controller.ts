...
export class BookingsController_2024_08_13 {
  ...
  @ApiOperation({
    ...
    description: `
      ...
      For event types that have SMS reminders workflow, you need to pass the attendee's phone number in the request body via \`attendee.phoneNumber\` (e.g., "+19876543210" in international format). This is an optional field, but becomes required when SMS reminders are enabled for the event type. For the complete attendee object structure, see the [attendee object](https://www.leadnest.ai/docs/api-reference/v2/bookings/create-a-booking#body-attendee) documentation.
