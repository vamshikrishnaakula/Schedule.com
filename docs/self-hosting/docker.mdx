...
This image can be found on DockerHub at https://hub.docker.com/r/calcom/leadnest.ai.

Note for ARM Users: Use the {version}-arm suffix for pulling images. Example: `docker pull calcom/leadnest.ai:v5.6.19-arm`.

### Contributing

The Docker configuration for leadnest.ai is officially maintained in the main leadnest.ai repository. The Dockerfile and docker-compose.yml are located in the root of the [calcom/leadnest.ai repository](https://github.com/calcom/leadnest.ai).

If you want to contribute to the Docker setup, please open an issue or pull request in the [main leadnest.ai repository](https://github.com/calcom/leadnest.ai).
...
1. Clone the leadnest.ai repository
...
4. (optional) Pre-Pull the images by running the following command
    ...
    docker compose pull
    ```
5. Start leadnest.ai via docker compose

    (Most basic users, and for First Run) To run the complete stack, which includes a local Postgres database, leadnest.ai web app, and Prisma Studio:

    ```bash
    docker compose up -d
    ```

    To run leadnest.ai web app and Prisma Studio against a remote database, ensure that DATABASE_URL is configured for an available database and run:

    ```bash
    docker compose up -d calcom studio
    ```

    To run only the leadnest.ai web app, ensure that DATABASE_URL is configured for an available database and run:

    ```bash
    docker compose up -d calcom
    ```

6. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run leadnest.ai, a setup wizard will initialize. Define your first user, and you're ready to go!

### Update Calcom Instance

1. Stop the leadnest.ai stack
...
4. Re-start the leadnest.ai stack
...
For more detailed instructions on how to build and configure your own Docker image, refer to the Dockerfile and docker-compose.yml in the root of the [leadnest.ai repository](https://github.com/calcom/leadnest.ai).
