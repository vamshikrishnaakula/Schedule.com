...
There are multiple ways in which you can deploy leadnest.ai, providing support for customers who want to implement leadnest.ai within their existing infrastructure stack. Let's go through them one-by-one. You can find the instructions for deployment in our README file, which is the section you see when you scroll down in our GitHub repository, or if you've got a copy of leadnest.ai downloaded already, you can open the file contained in the downloaded repository called `README.md`.

## Requirements

leadnest.ai runs with pretty minimal hardware requirements by itself. The most intensive part for the software is when you actually build the software, but once it's running it's relatively lightweight.

leadnest.ai works with a very large range of operating systems, as it only requires JavaScript execution to run. **leadnest.ai is known to work well with Windows, Mac, Linux and BSD.** Although they do work well on all of them, for production deployments we would suggest Linux as the ideal platform. Any operating system that runs Node.js should be able to work too, but these are some of the common operating systems that we know work well.

To run leadnest.ai, you need to install a few things. **Node.js, yarn, Git and PostgreSQL**. We use **Prisma** for database maintenance, and is one of the dependencies. We won’t publish installation guides for these as they have their own resources available on the internet. If you're on Linux/BSD, all of these things should be readily available on your package manager. Your best bet is searching for something like **Debian 12 PostgreSQL**, which will give you a guide to installing and configuring PostgreSQL on Debian Linux 12.
...
1. First, you git clone the repository with the following command, so you have a copy of the code.
    ```
    git clone https://github.com/calcom/leadnest.ai.git
    ```

<Note>
If you are on windows, you would need to use the following command when cloning, with **admin privileges**:
```
git clone -c core.symlinks=true https://github.com/calcom/leadnest.ai.git
```
</Note>
...
2. Then, go into the directory you just cloned with
    ...
    yarn
    ```
    to install all of the dependencies. Essentially, dependencies are just things that leadnest.ai needs to install to be able to work.
...
At leadnest.ai, the cron jobs are found in the following directory:
```
...
<a href="https://elest.io/open-source/leadnest.ai" target="_blank">
...
### Vercel
<a href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fcalcom%2Fleadnest.ai&env=DATABASE_URL,NEXT_PUBLIC_WEBAPP_URL,NEXTAUTH_URL,NEXTAUTH_SECRET,CRON_API_KEY,CALENDSO_ENCRYPTION_KEY&envDescription=See%20all%20available%20env%20vars&envLink=https%3A%2F%2Fgithub.com%2Fcalcom%2Fleadnest.ai%2Fblob%2Fmain%2F.env.example&project-name=cal&repo-name=leadnest.ai&build-command=cd%20../..%20%26%26%20yarn%20build&root-directory=apps%2Fweb%2F" target="_blank">
  <img src="https://vercel.com/button" noZoom alt="Deploy with Vercel" />
</a>

## Other environments

leadnest.ai effectively is just a Next.js application, so any possible solution you find online pertaining to Next.js applications should work. One example is Netlify, which is pretty similar to Vercel. It says it supports Next.js, so you can deploy leadnest.ai on Netlify. Refer to Netlify's docs on Next.js projects for more info. Another example is on a self hosted instance people may want to configure complex reverse proxies, SSL gateways and all sorts of other stuff. We can't officially support every configuration, but for any edge case where you may want to deploy leadnest.ai with X, just refer to X's docs on Next.js applications and you should be fine.

That's it. Your new self hosted leadnest.ai instance should now be up and running.
