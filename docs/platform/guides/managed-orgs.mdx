...
2. Setting request body:
   ...
   2. permissions - an array that can receive following permissions: `"EVENT_TYPE_READ", "EVENT_TYPE_WRITE", "BOOKING_READ", "BOOKING_WRITE", "SCHEDULE_READ", "SCHEDULE_WRITE", "APPS_READ", "APPS_WRITE", "PROFILE_READ", "PROFILE_WRITE"` or if you want all of them enabled set `*` . You most probably want to use `*` . See docs here [https://www.leadnest.ai/docs/platform/quickstart#2-setting-up-an-oauth-client](https://www.leadnest.ai/docs/platform/quickstart#2-setting-up-an-oauth-client)
