...
export const getUsersFromGWorkspace = async ({ ctx }: CheckForGCalOptions) => {
  ...
  const directory = new admin_directory_v1.Admin({
    // The `googleapis` packages may resolve their own copy of `google-auth-library`,
    // which can make the `OAuth2Client` type incompatible at compile time even
    // though it works at runtime. We cast here to satisfy TypeScript.
    auth: oAuth2Client as unknown as GlobalOptions["auth"],
  });
