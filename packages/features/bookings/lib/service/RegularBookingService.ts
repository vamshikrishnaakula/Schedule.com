...
import { getAppFromSlug, getSystemDefaultConferencingLocationType } from "@calcom/app-store/utils";
import {
...
async function handler(
  ...
  if (locationBodyString == OrganizerDefaultConferencingAppType) {
    ...
    if (organizerMetadata?.defaultConferencingApp?.appSlug) {
      ...
      locationBodyString = getSystemDefaultConferencingLocationType();
    }
  }
  ...
  if (!eventType.seatsPerTimeSlot && originalRescheduledBooking?.uid) {
    ...
    if (results.length > 0 && results.every((res) => !res.success)) {
      ...
      const { metadata: videoMetadata, videoCallUrl: createdVideoCallUrl } = getVideoCallDetails({
        results,
      });
      ...
      if (results.length) {
        ...
        additionalInformation.hangoutLink = videoMetadata.hangoutLink || results[0].createdEvent?.hangoutLink;
        additionalInformation.conferenceData =
          videoMetadata.conferenceData || results[0].createdEvent?.conferenceData;
        additionalInformation.entryPoints = videoMetadata.entryPoints || results[0].createdEvent?.entryPoints;
        evt.appsStatus = handleAppsStatus(results, booking, reqAppsStatus);
        videoCallUrl =
