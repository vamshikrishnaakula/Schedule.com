...
function getRandomBookingStatus() {
	const randomStatusIndex = Math.floor(
		Math.random() * VALID_STATUSES_FOR_SEED.length,
	);
...
const shuffle = (
	...
	const startTime = dayjs(booking.startTime)
		...
		.set("y", year);
	const randomEvent =
		eventTypesToPick[Math.floor(Math.random() * eventTypesToPick.length)];

	const endTime = dayjs(startTime).add(
		Math.floor(Math.random() * randomEvent.length),
		"minute",
	);
	...
	if (usersIdsToPick && usersIdsToPick.length > 0) {
		...
		booking.userId =
			usersIdsToPick[Math.floor(Math.random() * usersIdsToPick.length)];
...
async function createDiversePaymentBookings(organizationId: number) {
	...
	for (const { eventTypeIndex, count, scenario } of scenarios) {
		...
		for (let i = 0; i < count; i++) {
			...
			allScenarioBookings.push({
				...
				userId:
					insightsTeamMembers[
						Math.floor(Math.random() * insightsTeamMembers.length)
					].user.id,
				paid: scenario.paid!,
				rescheduled: false,
	...
	const bookingsForPayments: BookingWithPaymentData[] = createdBookings
		...
		.map((booking, index) => {
			const scenarioBooking =
				allScenarioBookings[allScenarioBookings.length - 1 - index]; // Reverse order since we sorted desc
			return {
				id: booking.id,
...
async function seedBookingAssignments() {
	...
	const randomBookings = bookings
		...
		.slice(0, numberOfBookingsToAssign);
...
async function main() {
	...
	if (!organization) {
		throw new Error(
			"Organization 'Acme Inc' not found. Please run seed.ts first.",
		);
	}
	...
	if (teamEvents?.length === 0) {
		...
		const assignAllTeamMembersEvents = teamEvents.filter(
			(event) => event.assignAllTeamMembers,
		);
		if (
			assignAllTeamMembersEvents.length > 0 &&
			insightsTeamMembers.length > 0
		) {
	...
	const javascriptEventId = teamEvents.find(
		(event) => event.slug === "team-javascript",
	)?.id;
	const salesEventId = teamEvents.find(
		(event) => event.slug === "team-sales",
	)?.id;
		...
		await prisma.booking.createMany({
			data: [
				...new Array(10000)
					.fill(0)
					.map(() =>
						shuffle({ ...baseBooking }, dayjs().get("y"), singleEventsCreated),
					),
			],
		});
	}
